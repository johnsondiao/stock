...
- **板块成色过滤 (2026-09-11 接入)**: ma_combo 命中后过 `sector_quality.py` ——
...
- 持仓纪律: 收盘跌破日线 MA12 减仓/清仓, MA60 为最后防线
  (止损规则回测 stop_loss_backtest.py 已验证: 现行规则优于"等确认", 不要改)
- **持仓口径三查(2026-09-14 立)**: pm_check.py 的 STOCKS 与 weekend_review.py 的
  HOLD 都会过期, 任一只都不可信。查持仓必须: ① 当天 `.workbuddy/memory/YYYY-MM-DD.md`
  的「当前持仓」小节 ② 根目录 `交易记录_*.md` 的持仓表 ③ pm_check.py STOCKS, 取最新
  (2026-09-14 14:45 实际持仓: **全柴动力300@7.953 + 深圳机场300@6.650**, 现金约4600;
   三峡水利/阳光照明/哈空调/铜陵有色/中国出版 均已清仓, 只在 WATCH 跟踪)
